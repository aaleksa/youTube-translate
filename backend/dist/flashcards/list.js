...
async function listFlashcards2(auth, params = {}) {
  ...
  const records = items.map(toRecord2).sort((left, right) => left.createdAt - right.createdAt);
...
// src/handlers/flashcards/list.ts
var handler = createProtectedHandler(async (event, auth) => {
  ...
  const cards = await listFlashcards2(auth, pagination);
  return ok(cards);
});
export {
  handler
