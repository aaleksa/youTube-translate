...
function isReviewDue(nextReview, now = Date.now()) {
...
function filterDueFlashcards(cards, now = Date.now()) {
...
function buildTodayReviewQueue(cards, now = Date.now()) {
...
// src/handlers/reviews/today.ts
var handler = createProtectedHandler(async (_event, auth) => {
  const reviews = await listTodayReviews(auth);
  return ok(reviews);
});
export {
  handler
