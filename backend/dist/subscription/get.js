...
function isPremiumSubscription(subscription, now = Date.now()) {
...
function getEffectivePlan(subscription, now = Date.now()) {
  return isPremiumSubscription(subscription, now) ? subscription.plan : "free";
}
function getEffectiveStatus(subscription, now = Date.now()) {
  if (subscription.endDate !== null && subscription.endDate < now) {
    return "expired";
  }
...
// src/handlers/subscription/get.ts
