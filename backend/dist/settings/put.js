...
async function updateUserSettings2(auth, input) {
  ...
  const current = existing && existing.userId === auth.userId ? toRecord2(existing) : defaultUserSettings(auth.userId);
  const merged = mergeUserSettings(current, validated);
  const now = Date.now();
  const item = {
...
// src/handlers/settings/put.ts
var handler = createProtectedHandler(async (event, auth) => {
  const body = await readBody(event);
  const settings = await updateUserSettings2(auth, body);
  return ok(settings);
});
export {
  handler
