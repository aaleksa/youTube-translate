...
import { buildChaptersPrompt } from '../../lib/aiPrompts';
import { resolveTaskLanguage } from '../../lib/aiInterfaceLanguage';
import {
  formatTranscriptForChapters,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const formatted = formatTranscriptForChapters(
    ...
    if (AI_PROVIDER === 'openai') {
      ...
      const message = await openai.chat.completions.create({
        model,
        messages: [
          { role: 'system', content: buildChaptersPrompt(language) },
      ...
      const parsed = parseChaptersResponse(raw);
    ...
    return NextResponse.json(
      {
        error:
          'Chapter generation currently requires OpenAI (AI_PROVIDER=openai)',
      },
      { status: 503 }
    );
  } catch (error) {
